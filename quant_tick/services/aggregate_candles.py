...
class AggregateCandleService:
    ...
    def aggregate(self, payloads: list[dict]) -> dict:
        all_params = []
        skipped = {"backoff": 0, "locked": 0}
        tasks = []
        ...
        if all_params and not tasks:
            ...
            return {"ok": True, "skipped": skipped_reason}
        ...
        try:
            ...
            for task_state, _items in tasks:
                if task_state.pk not in released:
                    task_state.release()
        return {"ok": True, "processed": processed}


def aggregate_candle_data(payloads: list[dict]) -> dict:
    return AggregateCandleService().aggregate(payloads)
