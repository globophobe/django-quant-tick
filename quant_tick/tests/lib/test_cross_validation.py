...
class PurgedKFoldTest(SimpleTestCase):
    ...
    def test_basic_split_without_event_end(self):
        ...
        bar_index = np.arange(len(X))
        cv = PurgedKFold(n_splits=5, embargo_bars=10)

        splits = list(cv.split(X, bar_index=bar_index))
    ...
    def test_purges_overlapping_events(self):
        ...
        splits = list(
            cv.split(
                X, event_end_exclusive_idx=event_end_exclusive_idx, bar_index=bar_index
            )
        )
    ...
    def test_preserves_time_order(self):
        ...
        prev_test_end = -1
        for _, test_idx in cv.split(X, bar_index=bar_index):
    ...
    def test_with_bar_index_for_interleaved_data(self):
        """Uses bar_index for purging when provided."""
        n_bars = 50
        n_configs = 3
        n_rows = n_bars * n_configs
        X = np.arange(n_rows).reshape(-1, 1)

        bar_index = np.repeat(np.arange(n_bars), n_configs)
        horizon = 10
        event_end_exclusive_idx = bar_index + horizon + 1
